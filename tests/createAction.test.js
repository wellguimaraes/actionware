...
describe('createAction', () => {
  ...
  describe('tracked action', () => {
    ...
    it('should dispatch success event when action executed successfully', () => {
      ...
      expect(dispatch.getCall(0).args[ 0 ].type).to.equal(trackedAction._successType);
    });

    it('should dispatch busy event when action is async', () => {
      const action = async () => Promise.resolve(1);
      const dispatch = spy();
      const trackedAction = createAction(action);
    ...
    //it('should dispatch error event when action failed', (done) => {
    //  const action = () => { throw new Error('Message') };
    //  const dispatch = spy();
    //  const trackedAction = createAction(action);
    //
    //  setStore({ dispatch, getState: spy() });
    ...
    //
    //  expect(dispatch.called).to.be.true;
    //  expect(dispatch.getCall(0).args[ 0 ].type).to.equal(trackedAction._errorType);
    //});
  })
